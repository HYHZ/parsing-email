package mailResolved.utils.javamail;
...
public class ReciveOneMail {
	...
	/**
	 * 解析邮件，把得到的邮件内容保存到一个StringBuffer对象中，解析邮件 主要是根据MimeType类型的不同执行不同的操作，
	 * 一步一步的解析
	 */
...
//		System.out.println("CONTENTTYPE: " + contenttype);
		...
		if (part.isMimeType("multipart/*")) {
			...
			for (int i = 0; i < mp.getCount(); i++) {
				...
				if ((disposition != null)
					...
					// add
					int count = mpart.getInputStream().available();
					byte[] b = new byte[count];
					int readCount = 0; // 已经成功读取的字节的个数
					while (readCount < count) {
						readCount += mpart.getInputStream().read(b, readCount,
								count - readCount);
					}
					
					byte[] getData =this.readInputStream( mpart.getInputStream());     //获得网站的二进制数据  
		            String data = new String(getData, "utf8");  
	...
	public static void main(String args[]) throws Exception {
		...
		URLName urln = new URLName("pop3", "pop3.163.com", 110, null,
				"lepintong@163.com", "lepintong201401");
